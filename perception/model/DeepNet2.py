...
def DeepNet(nb_classes, image_size, dropout_rate, weight_decay):
    ...
    output = Convolution2D(8, (1, 1), padding='same')(output)
    # downsampling layer (avg pool)
    output = AveragePooling2D((2, 2), strides=(1, 1))(output)
    output = Activation('relu')(output)
    ...
    for i in range(nb_classes):
        ...
        inter = Dense(1, activation="relu")(inter)
    ...
    # Hidden layer 1
    output = Dense(256, kernel_regularizer=regularizers.l2(weight_decay), 
                        bias_regularizer=regularizers.l2(weight_decay))(output)
    output = PReLU()(output)
    output = Dropout(dropout_rate)(output)
    
    softmax = Dense(nb_classes, activation='softmax')(output)
